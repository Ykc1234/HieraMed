...
class PatientGraph(torch.utils.data.Dataset):
    def __init__(self, tokenizer, subset, dim, device, trans_dim = 0, di = False):
        self.c_tokenzier = tokenizer['conds_hist']
    ...
    def _process_(self):
        ...
        for dp in tqdm(range(len(self.dataset))):
            ...
            dpc = self.c_tokenzier.batch_encode_2d(self.dataset[dp]['conds_hist'], padding=False)
            ...
            data['co'].x = torch.zeros(1,self.dim)
            data['pr'].x = torch.zeros(1,self.dim)
            data['dh'].x = torch.zeros(1,self.dim)
...
class PatientGraphSequeces(torch.utils.data.Dataset):
    def __init__(self, tokenizer, subset, dim, device, trans_dim=0, di=False):
        self.c_tokenzier = tokenizer['conds_hist']
        self.d_tokenzier = tokenizer['drugs']
        self.p_tokenzier = tokenizer['procedures']
    ...
    def _process_(self):
        ...
        for dp in tqdm(range(len(self.dataset))):
            graph_list = []
            num_visit = len(self.dataset[dp]['procedures'])
              # self.visits[dp].unsqueeze(0).expand(num_visit, -1) #torch.zeros(num_visit, self.dim)
            dpc = self.c_tokenzier.batch_encode_2d(self.dataset[dp]['conds_hist'], padding=False)
            dpp = self.p_tokenzier.batch_encode_2d(self.dataset[dp]['procedures'], padding=False)
            dpd = self.d_tokenzier.batch_encode_2d(self.dataset[dp]['drugs'], padding=False)

            #data['visit'].time = convert_to_relative_time(self.dataset[dp]['adm_time'])
            #print(self.dataset[dp]['adm_time'])
            #print(1111111)
            for i in range(num_visit):
                data = HeteroData()
                #data['visit'].x = torch.zeros(num_visit, self.dim)
                #print(self.dataset[dp]['adm_time'][i])
                data['visit'].x = torch.zeros(num_visit, 1)
                data['visit'].time = convert_to_relative_time([self.dataset[dp]['adm_time'][i]])
                data['co'].x = torch.zeros(self.c_tokenzier.get_vocabulary_size(), 1)
                data['pr'].x = torch.zeros(self.p_tokenzier.get_vocabulary_size(), 1)
                data['dh'].x = torch.zeros(self.d_tokenzier.get_vocabulary_size(), 1)
                ...
                civ = torch.tensor([[item for item in dpc[i]],
                                    [i for item in dpc[i]]], dtype=torch.int64)
                data['co', 'in', 'visit'].edge_index = civ
                piv = torch.tensor([[item for item in dpp[i]],
                                    [i for item in dpp[i]]], dtype=torch.int64)
                data['pr', 'in', 'visit'].edge_index = piv
                div = torch.tensor([[item for item in dpd[i]],
                                    [i for item in dpd[i]]], dtype=torch.int64)
                data['dh', 'in', 'visit'].edge_index = div
                viv = torch.tensor([[i], [i]],
                                   dtype=torch.int64)
                data['visit', 'connect', 'visit'].edge_index = viv
                ...
                data['co', 'in', 'visit'].edge_time = torch.tensor(
                    [i for index in dpc[i]], dtype=torch.float32)
                data['pr', 'in', 'visit'].edge_time = torch.tensor(
                    [i for index in dpp[i]], dtype=torch.float32)
                data['dh', 'in', 'visit'].edge_time = torch.tensor(
                    [i for index in dpd[i]], dtype=torch.float32)
