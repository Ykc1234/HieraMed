...
def get_init_tokenizers(task_dataset, keys = ['conds_hist', 'procedures', 'drugs']):
    Tokenizers = {key: Tokenizer(tokens=task_dataset.get_all_tokens(key), special_tokens=["<pad>"]) for key in keys}
    return Tokenizers

def get_parent_tokenizers(task_dataset, keys = ['conds_hist', 'procedures']):
    parent_tokenizers = {}
...
def train(data_loader, model,label_tokenizer, optimizer, device):
    ...
    for data in data_loader:
        ...
        if type(data) == dict:
            ...
            label = torch.tensor(data[0]['label']).to(device)
        '''
        print(data[0]['conditions'])
        if type(data)==dict:
        ...
        out = model(data)
        loss = F.binary_cross_entropy_with_logits(out,label.float(),pos_weight=pos_weight.to(device))
...
def topk_to_binary(tensor, k):
    topk_indices = torch.topk(tensor, k, dim=1).indices
    #binary_tensor = torch.zeros_like(tensor, dtype=torch.int)
    #binary_tensor.scatter_(dim=1, index=topk_indices, value=1)
    return topk_indices.tolist()


def plot(data_loader, model, label_tokenizer, seed):
    x_cond, x_proc, x_drug = [], [], []
    with torch.no_grad():
        for data in tqdm(data_loader):
            ...
            if type(data) == dict:
                ...
                label = torch.tensor(data[0]['label'])
            '''
            if type(data) == dict:
            ...
            # print(data)
            out = model(data)
            out = topk_to_binary(out, 20)
            outlist = label_tokenizer.batch_decode_2d(out, padding=False)

            n = 0

            for i in range(len(data[0]['conds_hist'])):
                print(1)
                if len(data[0]['conds_hist'][i]) == 3 and n <= 15:
                    print(data[0]['patient_id'][i])
                    print(data[0]['conds_hist'][i])
                    print(data[0]['conditions'][i])
                    print(outlist[i])
                    print(data[0]['procedures'][i])
                    print(data[0]['drugs'][i])
                    n = n + 1
                    print('###################')


            '''
            list_conds, list_proc, list_drugs, out_conds, out_proc, out_drugs = model(data)
            conds_list = list_conds
            proc_list = list_proc
            drugs_list = list_drugs
            conds_features = out_conds.detach().cpu()
            proc_features = out_proc.detach().cpu()
            drugs_features = out_drugs.detach().cpu()
            ...
            # 保存每一类
            save_embeddings_to_csv(list_conds, conds_features, 'conds_embeddings.csv')
            save_embeddings_to_csv(list_proc, proc_features, 'proc_embeddings.csv')
            save_embeddings_to_csv(list_drugs, drugs_features, 'drugs_embeddings.csv')
...
def test(data_loader, model, label_tokenizer):
    ...
    with torch.no_grad():
        for data in tqdm(data_loader):
            ...
            for idx, sample in enumerate(data[0]['conds_hist']):
                visit_count = len(sample)
                visit_dict.append(visit_count)
        ...
        for idx, vc in enumerate(visit_dict):
            visit_groups[vc].append(idx)
        for vc, indices in visit_groups.items():
            yt = y_true[indices]
            yp = y_prob[indices]
            print(vc)
            if vc < 11:
                print(code_level(yt, yp))
                print(visit_level(yt, yp))
